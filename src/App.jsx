...
export default function App() {
  ...
  const premierProduitDisponibleId = useMemo(() => produits.find((p) => Number(p.quantite || 0) > 0)?.id || "", [produits]);
  ...
  async function saveVente(e) {
    ...
    setVenteForm({ produit_id: premierProduitDisponibleId, client_id: "", quantite: 1, type_vente: "comptant", mode_paiement: "Espèces", montant_paye: "", date_echeance: "" });
    setVenteLignes([{ produit_id: premierProduitDisponibleId, quantite: 1 }]);
  ...
  function imprimerFacture(v) {
    ...
    const lignes = isGroupe
      ? v.lignes
      : ventes
          .filter((x) => factureReferenceGroupe(x) === ref && x.client_id === v.client_id)
          .map((x) => ({
            ...x,
    ...
    const lignesFinales = (lignes.length ? lignes : [{
      ...v,
      produit: produits.find((x) => x.id === v.produit_id),
      total: Number(v.quantite || 0) * Number(v.prix_unitaire || 0)
    }]).map((l) => ({
      ...l,
      total: Number(l.total ?? (Number(l.quantite || 0) * Number(l.prix_unitaire || 0)))
    }));

    const c = v.client || clients.find((x) => x.id === v.client_id);
    const total = lignesFinales.reduce((s, l) => s + Number(l.total || 0), 0);
    ...
    const dateFacture = v.date_vente || String(v.created_at || "").slice(0, 10) || today();
    const statutFacture = reste <= 0 ? "Payée" : "Partiellement payée";

    const rows = lignesFinales.map((l, idx) => {
      const nomProduit = l.produit?.nom || produits.find(p => p.id === l.produit_id)?.nom || "Produit";
      const qte = Number(l.quantite || 0);
      const pu = Number(l.prix_unitaire || 0);
      const totalLigne = Number(l.total || (qte * pu));
      return `
      <tr>
        <td class="num">${idx + 1}</td>
        <td>
          <strong>${safeText(nomProduit)}</strong>
          ${l.produit?.code_produit ? `<div class="muted">Code : ${safeText(l.produit.code_produit)}</div>` : ""}
        </td>
        <td class="right">${safeText(qte)}</td>
        <td class="right">${safeText(fmt(pu))}</td>
        <td class="right total-col">${safeText(fmt(totalLigne))}</td>
      </tr>`;
    }).join("");
          ...
          <style>
            * { box-sizing: border-box; }
            ...
            .page {
              ...
              border-radius: 22px;
              overflow: hidden;
              box-shadow: 0 28px 80px rgba(21,34,56,.18);
            }
            .hero {
              position: relative;
              display: flex;
              ...
              background: linear-gradient(135deg, #152238 0%, #1E7F6E 72%, #E0913C 100%);
              color: #fff;
            }
            .hero:after {
              content: "";
            ...
            .brand .infos { margin-top: 10px; font-size: 12.5px; line-height: 1.65; color: rgba(255,255,255,.86); max-width: 420px; }
            .invoice-title { text-align: right; position: relative; z-index: 1; }
            .invoice-title h2 { margin: 0; font-size: 38px; letter-spacing: 1.4px; }
            .invoice-title .ref { margin-top: 12px; display: inline-block; padding: 8px 12px; border-radius: 999px; background: rgba(255,255,255,.16); font-weight: 900; }
            .content { padding: 30px 42px 38px; }
            .top-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; margin-bottom: 26px; }
            .box { border: 1px solid #e5eaf0; border-radius: 16px; padding: 16px 18px; background: #fbfdff; min-height: 125px; }
            .box-title { color: #1E7F6E; font-size: 11px; font-weight: 900; text-transform: uppercase; letter-spacing: .7px; margin-bottom: 10px; }
            .line { font-size: 13.3px; line-height: 1.55; margin: 4px 0; }
            .muted { color: #64748b; font-size: 11.5px; margin-top: 3px; }
            table { width: 100%; border-collapse: collapse; border-radius: 16px; overflow: hidden; border: 1px solid #e5eaf0; }
            th { background: #152238; color: #fff; text-align: left; padding: 13px 14px; font-size: 11.5px; text-transform: uppercase; letter-spacing: .45px; }
            td { padding: 14px; border-bottom: 1px solid #eef2f6; font-size: 13.5px; vertical-align: top; }
            tbody tr:nth-child(even) { background: #f8fafc; }
            .num { width: 44px; color: #64748b; font-weight: 900; }
            .right { text-align: right; white-space: nowrap; }
            .total-col { color: #1E7F6E; font-weight: 900; }
            .summary { display: grid; grid-template-columns: 1fr 380px; gap: 22px; margin-top: 24px; align-items: start; }
            .note { background: #fff7ed; border: 1px solid #fed7aa; border-radius: 16px; padding: 16px; color: #92400e; font-size: 12.8px; line-height: 1.55; }
            .total-box { border: 2px solid #1E7F6E; border-radius: 18px; overflow: hidden; background: #fff; }
            .total-box .row { display: flex; justify-content: space-between; padding: 12px 16px; border-bottom: 1px solid #e5eaf0; font-size: 13.5px; }
            .total-box .grand { display: flex; justify-content: space-between; align-items: center; padding: 16px; background: #ecfdf5; color: #0f766e; font-size: 22px; font-weight: 1000; }
            .status { display: inline-block; padding: 5px 9px; border-radius: 999px; background: #ecfdf5; color: #0f766e; font-size: 12px; font-weight: 900; }
            .signature { display: flex; justify-content: space-between; margin-top: 42px; gap: 20px; }
            .signature > div { flex: 1; text-align: center; border-top: 1px solid #94a3b8; padding-top: 10px; color: #64748b; font-size: 12.5px; }
            .footer { margin-top: 28px; text-align: center; color: #64748b; font-size: 12.2px; line-height: 1.6; }
            @media print {
              body { background: #fff; }
              .page { width: auto; margin: 0; border-radius: 0; box-shadow: none; }
          ...
          <div class="page">
            <div class="hero">
              ...
              <div class="invoice-title">
                <h2>FACTURE</h2>
                <div class="ref">N° ${safeText(ref)}</div>
                <div style="margin-top:10px;font-size:13px;color:rgba(255,255,255,.86)">Date : ${safeText(formatDateFr(dateFacture))}</div>
              </div>
            </div>
            ...
            <div class="content">
              <div class="top-grid">
                ...
                <div class="box">
                  ...
                  <div class="line"><strong>Mode paiement :</strong> ${safeText(v.mode_paiement || "Espèces")}</div>
                  <div class="line"><strong>Nombre de produits :</strong> ${lignesFinales.length}</div>
                  <div class="line"><strong>Statut :</strong> <span class="status">${safeText(statutFacture)}</span></div>
                </div>
              </div>
              ...
              <div class="summary">
                <div class="note">
                  <strong>Note :</strong> cette facture regroupe tous les produits de la vente sous une seule référence. Merci de conserver ce document pour votre suivi.
                </div>
                <div class="total-box">
                  <div class="row"><span>Montant payé</span><strong>${safeText(fmt(montantPaye))}</strong></div>
                  <div class="row"><span>Reste à payer</span><strong>${safeText(fmt(reste))}</strong></div>
                  <div class="grand"><span>Total TTC</span><span>${safeText(fmt(total))}</span></div>
                </div>
              </div>
              ...
              <div class="footer">
                ...
                Document généré automatiquement par Suivi PME.
              </div>
            </div>
          </div>
